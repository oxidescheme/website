import type { Metadata } from "next";
import { coreContributors } from "@/data/contributors";
import { getContributors, getOrgStats } from "@/lib/github";

export const metadata: Metadata = {
  title: "community",
};

export const revalidate = 86400; // 24 hours

interface FetchedContributor {
  login: string;
  name: string;
  avatarUrl: string;
  githubUrl: string;
  contributions: number;
  repos: string[];
}

export default async function CommunityPage() {
  const fetchedContributors = await getContributors();
  const { totalStars, totalPorts } = await getOrgStats();

  // Filter out core contributors from fetched contributors to avoid duplicates
  const coreUsernames = new Set(coreContributors.map((c) => c.username));
  const otherContributors = fetchedContributors.filter(
    (c: FetchedContributor) => !coreUsernames.has(c.login),
  );

  const totalContributors = coreContributors.length + otherContributors.length;

  return (
    <main className="max-w-6xl mx-auto px-6 py-12 flex-1 w-full">
      <div className="animate-fadeIn">
        <div className="mb-12">
          <h2 className="font-mono text-4xl font-bold text-bright-text mb-2">
            Community
          </h2>
          <p className="text-subtext0">
            The people behind oxide. Whether you maintain a port or contributed
            via pull request — you make this possible.
          </p>
        </div>

        {/* Stats */}
        <div className="grid grid-cols-3 gap-4 mb-12">
          {[
            { label: "GitHub Stars", value: totalStars },
            { label: "Active Ports", value: totalPorts },
            { label: "Contributors", value: totalContributors },
          ].map((stat) => (
            <div
              key={stat.label}
              className="border border-subtext2 bg-surface0 p-5"
            >
              <div className="font-mono text-3xl font-bold text-bright-text mb-1">
                {stat.value.toLocaleString()}
              </div>
              <div className="text-xs text-subtext1 uppercase tracking-wider">
                {stat.label}
              </div>
            </div>
          ))}
        </div>

        {/* Core Team */}
        <div className="mb-12">
          <div className="font-mono text-xs text-subtext1 uppercase tracking-wider mb-4">
            Core Team
          </div>
          <div className="grid grid-cols-3 gap-4">
            {coreContributors.map((contributor) => (
              <a
                key={contributor.id}
                href={contributor.githubUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="border border-subtext2 bg-surface0 p-5 hover:border-subtext1 transition-colors group"
              >
                <div className="flex items-center gap-4 mb-4">
                  <img
                    src={contributor.avatarUrl}
                    alt={contributor.name}
                    className="w-12 h-12 grayscale group-hover:grayscale-0 transition-all"
                  />
                  <div>
                    <div className="font-mono font-semibold text-bright-text group-hover:text-blue transition-colors">
                      {contributor.name}
                    </div>
                    <div className="text-xs text-subtext1">
                      @{contributor.username}
                    </div>
                  </div>
                </div>
                <div className="text-sm text-subtext0 mb-3">
                  {contributor.role}
                </div>
                <div className="flex flex-wrap gap-2">
                  {contributor.contributions.map((contrib: string) => (
                    <span
                      key={contrib}
                      className="text-xs bg-surface1 text-subtext0 px-2 py-1"
                    >
                      {contrib}
                    </span>
                  ))}
                </div>
              </a>
            ))}
          </div>
        </div>

        {/* Contributors */}
        {otherContributors.length > 0 && (
          <div>
            <div className="font-mono text-xs text-subtext1 uppercase tracking-wider mb-4">
              Contributors ({otherContributors.length})
            </div>
            <div className="grid grid-cols-4 gap-4">
              {otherContributors.map((contributor: FetchedContributor) => (
                <a
                  key={contributor.login}
                  href={contributor.githubUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="border border-subtext2 bg-surface0 p-4 hover:border-subtext1 transition-colors group"
                >
                  <div className="flex items-center gap-3 mb-3">
                    <img
                      src={contributor.avatarUrl}
                      alt={contributor.name}
                      className="w-10 h-10 grayscale group-hover:grayscale-0 transition-all"
                    />
                    <div>
                      <div className="font-mono font-semibold text-bright-text text-sm group-hover:text-blue transition-colors">
                        {contributor.name}
                      </div>
                      <div className="text-xs text-subtext1">
                        @{contributor.login}
                      </div>
                    </div>
                  </div>
                  <div className="text-xs text-subtext0">
                    {contributor.contributions} contributions
                  </div>
                </a>
              ))}
            </div>
          </div>
        )}

        {/* Links */}
        <div className="border border-subtext2 bg-surface0 p-6 mt-12">
          <div className="font-mono text-xs text-subtext1 uppercase tracking-wider mb-4">
            Get Involved
          </div>
          <div className="flex gap-6">
            <a
              href="https://github.com/oxidescheme"
              className="flex items-center gap-2 text-subtext0 hover:text-bright-text transition-colors"
            >
              <svg
                className="w-5 h-5"
                fill="currentColor"
                viewBox="0 0 24 24"
                aria-hidden="true"
              >
                <title>GitHub icon</title>
                <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
              </svg>
              <span className="font-mono text-sm">GitHub Organization</span>
            </a>
            <a
              href="https://discord.gg/oxidescheme"
              className="flex items-center gap-2 text-subtext0 hover:text-bright-text transition-colors"
            >
              <svg
                className="w-5 h-5"
                fill="currentColor"
                viewBox="0 0 24 24"
                aria-hidden="true"
              >
                <title>Discord icon</title>
                <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z" />
              </svg>
              <span className="font-mono text-sm">Discord Server</span>
            </a>
          </div>
        </div>
      </div>
    </main>
  );
}
